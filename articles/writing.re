...
たとえばじゃんけんの説明をするのであれば、「2人以上でやる遊び」「日本ではみんなが知っている」「勝敗や順番をさっと決めたいときによくやる」といった概要をまず書きます。その上で、「グーとパーとチョキという3種類の手がある」「グーはチョキに、チョキはパーに、パーはグーに勝てるという3すくみの関係になっている」という詳細を書き、最後に「最初はグー、といかけ声で始めることが多い」「グーは石、チョキはハサミ、パーは紙を象徴している」のような補足情報を書きます。
...
@<ttb>{まず、事前に確認しておきたいのですが先週金曜の16時の定例のミーティングで話に上がっていた方針で間違いはないと思うのですが、興味深い内容がありましたのでその際にもお伝えしたとおりその件につきまして理解したとおりの見解を共有させていただければという意図です。既存仕様と異なると考えられる理由としては殆どの機能がメインケースとサブケースを切り分けて動作していないので意図と異なる動きになりユーザーが混乱すると考えているため、リジェクトの可能性もあるので回避のために僭越ながら先方への回答は迅速に行いたいと思っています。}
