...
最初に「想定する読者層」および「マッチしない読者層」、そして「ゴール」を書いておくことで、不幸なミスマッチを防ぎましょう。これらを書いておくと、著者自身が文章の方向性を見失いかけたときに「これって誰に向けた文章なんだっけ？」「読み終わったらどうなってて欲しいんだっけ？」と振り返る拠り所にもなります。
...
何の食べ物だか言わずにいきなり「食べて！ほら食べて！」とスプーンを差し出されると、「え、怖い。なになになに？」となって、とても素直に口を開く気にはなれませんし、食べたところで猜疑心で味もよく分かりません。そんなときは「初めて作ったプリンが思いのほか美味しくできたので一口食べて感想を教えてほしい」というように、「どういう気持ちで何をして欲しいのか」を先に説明してあげる必要があります。
...
たとえばババ抜きの説明を書くのであれば、「トランプを使うゲーム」「2人以上でやる」といった大枠をまず書きます。その上で、「同じ数字のカードは2枚を1組にして場に捨てられる」「カードの中に1枚だけジョーカーがある」「順番に隣の人のカードを引いていき、最後まで手元にジョーカーが残った人が負け」というゲームのルールを書きます。そして最後に「どれがジョーカーか悟られないようにポーカーフェイスを保つことが大事」「ババ抜きから派生したゲームでジジ抜きもある」のような補足情報を添えます。
...
技術ドキュメントを読んでいても、最初から知らない単語や知らない概念ばかりが出てくると、「知らないことについて説明してくれているけど、その説明がまず分からない」という状態になります。まずは読み手がすでに知っていることから始めて、段々と知らないことに繋げていきましょう。
...
まずは知っていることからはじめて、段々と知らないことへ、認知の負荷は順を追って少しずつ高くしていきましょう。

=== 1つの段落では1つのことだけ書こう

人はみんな忘れる生き物です。2時間観た映画がとても面白かったとしても、終わった後に主人公の名前が思い出せない、みたいなことはざらにあります。

分かってほしいからこそ、あれこれも色々書きたくなってしまうのですが、いま書いているこの段落でつたえたいことは何だったかを考えて、1つの段落に書くことは1つだけに絞りましょう。もし1つの段落で2つのことを混ぜて説明していたら、それは2つの段落に分ければいいだけです。
...
数があまりに多い場合は、まず項目を階層化しましょう。前述のデザートの例なら「デザートは温かいものと冷たいものがそれぞれ5つずつあります」で一度分岐させてから、選んだ方の5種類を提示することで全体が把握しやすくなります。
...
ドキュメントはできるだけGitなどでバージョン管理できる状態にすることで、本当に必要なものと古い情報だが取っておきたいというものが同じ場所に置かれていることのないようにしましょう。

たとえば情報共有サービスのesa@<fn>{esa}では、「アーカイブ」という機能があります。古い情報だが念のため取っておきたい記事はアーカイブしておくことで、検索した際にんその記事の検索順位を落とすことができます。
...
自転車は漕ぎはじめはペダルが重くてしんどいですが、速度が乗ってくれば少しの労力でぐいぐい進みます。文章も同じで、0から書く部分がいちばん重たくて時間がかかります。大抵の人は0から作るよりも他人が作ったものにああだこうだ文句を付ける方が圧倒的に得意なので、書きはじめの大変なところだけ生成AIにやってもらう、という方法があります。

文章を書こうと思ってパソコンの前に座ったが、どうも書きあぐねている……というときは、雑な指示をChatGPTに出したり、GitHub Copilotの提案に身を委ねたりして、低品質でいいので初案を作ってもらいましょう。（@<img>{chatgpt}）

//image[chatgpt][雑な指示を出して初案を書いてもらう][scale=1]{
//}

初案を読んで「これ、言いたいこととはちょっと違うし、あれこれ間違っていてひどいな」と思ったらしめたものです。そうじゃなくてこうだよ、とサクサク手直しをしていきましょう。悩むばかりで1文字も進まないよりは、ずっと速くあなたの文章ができあがるはずです。

=== 長文を書きたいならまずは一文から

包丁すら握ったことがない人が、いきなりフレンチのフルコースを作ろうとすれば大抵は失敗します。それと同じで、短い文章すら書き慣れていないのにいきなり壮大なドキュメントを書こうとすると、うまくいかずにやたらと時間がかかってしまいます。

まずはトマトを切って塩を振っただけのものを毎日作ってみる。それがうまくできるようになったら、トマトときゅうりを切ってサラダにしてみる。それも慣れてきたらサラダとは別に肉を焼いてみたり、ご飯も炊いてみたりする。そこまできたらついに献立を考えて朝昼晩の食事を作ってみる、というように意識してゆっくりと難易度を上げていく必要があります。
...
「ここの文章、いまいち分かりにくいから直したいんだけど、どうしようかな」と悩みながら書き足したり、消したり、少し言い回しを変えたり、前後を入れ替えたり……いやいややっぱりさっきの方がよかったかも、と元に戻したり……というシチュエーションに心当たりはありませんか？

いちばんよい一文を探してああでもないこうでもないと長時間悩んでしまう人には、たくさんのパターンを並べて書き、その中から最善手を見つけるという方法がお勧めです。

たとえば「メッセージ送信で文字数の上限が100文字から200文字に変更された」ということを伝える文章を書きたいとき、初案として次のような一文を書いたとします。
...
これが長時間悩んでしまうダメなパターンです。書き換えてしまうと初案と第2案の比較ができません。そして、まだ初案と第2案の2つくらいなら自分がどこを変えたのかや原型はなんだったのかを覚えていられますが、ここからあれこれ悩みながら書き足したり、消したり、少し言い回しを変えたり、前後を入れ替えたりしていると、「え、さっきいいなって思ったのはどれだったっけ？」となる未来が待っています。

いちばんよい一文を考えるときは、ヒストリーと差分が見えるように直前の案をコピーペーストしながら書き換えて、全部の案を並べていきましょう。このとき自分ひとりで考えるのもいいですが、誰かに付き合ってもらってオンラインミーティングで画面共有をしながら考えていくと、改善の螺旋階段を上がってく速度が急激に速くなり、ものの5分くらいで「これにしよう！」といういちばんよい一文に辿りつけます。
...
併せてそれぞれの案に数字を振っておくこともお勧めします。比較対象が2つくらいなら「上の方が分かりやすい」とか「私は下の方がいいと思う」で済みますが、3つ以上になったときに「上からえーっと……4つめのやつよりは、その下の下のやつがいいと思う」みたいになって非常に辛いです。こんなふうに案を並べて書きながらそれぞれに数字を振っておくことで、「4がいいと思う」「4よりは6の方が分かりやすい」のように、他の人に伝えやすくなります。

===[column] 【コラム】変わっていく語感を拾おう

いま、若い世代に「1時間弱は何分ぐらいのことか？」と聞くと、「1時間が60分で、1時間とちょびっとだから70分くらい？」と答えるらしい。まさかと思って息子に聞いたら、たしかにまったく同じように答えたのでちょっと驚きました。

その理屈でいくと1時間強は何分になるのかと聞いたところ、「1時間とたくさんだから、85分くらいとか？」らしい。なるほど。同じ理屈で大さじ1杯弱についても「大さじ1杯プラスちょっと」だと思っていたとのこと。
...
たとえば学校で先生に当てられて答えを言ったとき、途中で先生から「はい。結構です」と止められたら、「回答に満足しました。そこまでで十分ですよ」というプラスの意味で受け取るか、「もういいです。それ以上聞きたくありません」というマイナスの意味で受け取るかは、に人や世代によってかなり異なると思います。

「誤解して受け取る側が悪いので、言葉の意味をちゃんと勉強してください」と憤る気持ちも分かりますが、誤解する人が2割、3割と増えていってもなお、書き手が頑なにその存在を無視し続けるのはよくないと思っています。前述のようなケースであれば、誤解している人が若い世代に多いと分かった時点で、「1時間弱というのは、1時間より少し短いという意味です」というような補足を入れてあげるか、曖昧な言い方をやめて「45分から60分」のように誤解されない説明にしましょう。

===[/column]

== 再利用しやすい文章にする

社内Wikiで書いた文章を社外ブログに流用したり、登壇用に作った発表原稿をドキュメントで使ったり、文章は何かしらの形で再利用されることがあります。そんなときに再利用しやすい形になっていると、書き直す労力が少なくて済みます。

=== 文章と修飾を分けて書こう

WordPressのようなCMSやMicrosoft Wordなどで書かれた文章は、文章そのものと「見出し」や「太字」といった修飾が一体化しているため、再利用する際の再加工に手間がかかります。

可能であればMarkdown@<fn>{markdown}やRe:VIEW@<fn>{review}などを使って、プレーンテキストで文章と修飾を分けて書いておくことで、別の環境に引っ越すときや文章を再利用するときに書き直す労力が少なくて済みます。

//footnote[markdown][文章を書くための軽量マークアップ言語のひとつ。オリジナルのMarkdownの他に、拡張されたMarkdown方言が存在する。たとえばGitHubのコメントなどで使えるMarkdownも方言の1つ。]
//footnote[review][Re:VIEW（リビュー）とは、Re:VIEWの記法に沿ってテキストの原稿を書き、コマンドをたたくと書籍として組版されたPDFやEPUBが生まれてくる素晴らしいツール。本書の原稿もRe:VIEWの記法で書かれています。詳しく知りたい方は「技術書をかこう！ ～はじめてのRe:VIEW～ 第3版」という書籍がおすすめです。 @<href>{https://techbooster.booth.pm/items/5231699}]
...
たとえばShops APIというAPIのドキュメントを書くとき、概要のページをただの「概要」にすべきか、それとも「Shops APIの概要」にすべきか、迷うことがあります。

概要のページはShops APIというカテゴリの下にあるので、ただの「概要」でも上位のタイトルを見れば補完されて「ああ、Shops APIの概要なんだな」と分かりますし、いちいち全部のページのタイトルで「Shops APIの概要」や「Shops APIの開発ガイドライン」や「Shops APIの制限事項」というように連呼するのはいささか冗長にも感じます。

ですが、いちいち上位のカテゴリ名を見なければ、何の概要だか分からないというのは不便なものです。また、いまは概要のページがShops APIというカテゴリの下にあっても、今後ドキュメントの構造が変わってすべてのAPIが一目で把握できるように各APIの概要だけが1つの階層に集められる可能性もあります。

一長一短ではありますが、筆者はページのタイトルだけを見て分かるようにしたいので、「Shops APIの概要」にする派です。
...
読み手にとっても、リンクを開いてみるまでどこに飛ばされるのかが分からないため、そのリンクを開いてみるべきか見なくてもよさそうかが判断できません。

リンクテキストは安易に「こちら」にせず、@<code>{詳しくはShops APIの開発ガイドをご覧ください}のように、リンク先のページやコンテンツの名前を書くようにしましょう。
...
//footnote[link][@<href>{https://twitter.com/mainichi_kotoba/status/1769229909283778901}]

URLを誰かに教えてあげるためにコピーペーストするときなら、「Slackのチャンネルに興味深い記事のURLを貼る」のように「貼る」でいいと思いますが、蜘蛛がこちらからあちらへ糸を張るように、他の情報と繋ぐためのリンクであれば、筆者は「商品一覧から詳細ページにリンクを張る」のように「張る」を使っています。
